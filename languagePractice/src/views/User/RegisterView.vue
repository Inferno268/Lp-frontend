<template>
  <form @submit.prevent="submitForm" class="max-w-sm mx-auto mt-10">
    ...
    <div class="mb-4">
      ...
      <input type="email" v-model="formData.email" required class="w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring focus:ring-opacity-50" />
    </div>
...
export default {
  ...
  methods: {
    ...
    sendDataToAPI(jsonData) {
      const apiUrl = 'http://localhost:1337/user/register';
